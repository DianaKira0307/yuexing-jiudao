...
program lunar_trajectory
    ...
    open(newunit=fout, file='data/traj_ecliptic.txt', status='replace', action='write')
    ...
    do i = 1, n_total
        ...
        write(fout, '(f12.4, 2x, f12.6, 2x, f12.6)') &
            day_i, lam * RAD2DEG, beta_val * RAD2DEG
    end do
    close(fout)

    deallocate(buf)
    write(*, '(a, i0, a)') 'Written ', n_total, ' points  →  data/traj_ecliptic.txt'
